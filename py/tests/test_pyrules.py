...
# --- TBX009 erc006-fingerprint-secret-arg (native secret-arg fingerprint) ---

# A declared reporter and one call site in the same file; declared v.py#report_it.
_DEF = "def report_it(*a):\n    print(a)\n\n\n"
_TBX009 = "TBX009 erc006-fingerprint-secret-arg"


def _lint_reporter(tmp_path, body, *, reporters="v.py#report_it"):
    _write(tmp_path, "v.py", _DEF + body)
    return _flake8(tmp_path, "v.py", reporters=reporters)


def test_secret_positional_identifier_flags(tmp_path):
    r = _lint_reporter(tmp_path, "def h(auth_token):\n    report_it(auth_token)\n")
    assert r.returncode == 1 and _TBX009 in r.stdout, f"{r.stdout}\n{r.stderr}"


def test_secret_in_fstring_flags(tmp_path):
    r = _lint_reporter(tmp_path, 'def h(api_key):\n    report_it(f"refresh {api_key}")\n')
    assert r.returncode == 1 and "TBX009" in r.stdout, r.stdout


def test_secret_in_dict_value_flags(tmp_path):
    # Tags dict: the VALUE is a secret-named identifier.
    r = _lint_reporter(
        tmp_path, 'def h(session_token):\n    report_it("msg", {"tok": session_token})\n'
    )
    assert r.returncode == 1 and "TBX009" in r.stdout, r.stdout


def test_secret_via_concatenation_flags(tmp_path):
    r = _lint_reporter(tmp_path, 'def h(auth_token):\n    report_it("x: " + auth_token)\n')
    assert r.returncode == 1 and "TBX009" in r.stdout, r.stdout


def test_secret_via_attribute_flags(tmp_path):
    r = _lint_reporter(tmp_path, "def h(cfg):\n    report_it(cfg.secret)\n")
    assert r.returncode == 1 and "TBX009" in r.stdout, r.stdout


def test_secret_via_format_arg_flags(tmp_path):
    r = _lint_reporter(tmp_path, 'def h(auth_token):\n    report_it("tok={}".format(auth_token))\n')
    assert r.returncode == 1 and "TBX009" in r.stdout, r.stdout


def test_substring_identifier_flags(tmp_path):
    # opengrep substring semantics: `tokens` contains `token`, matches.
    r = _lint_reporter(tmp_path, "def h(tokens):\n    report_it(tokens)\n")
    assert r.returncode == 1 and "TBX009" in r.stdout, r.stdout


def test_qualified_reporter_call_secret_flags(tmp_path):
    # The bug the native rule fixes: opengrep matched bare names only, so a
    # qualified `report.sentry_err(...)` slipped past. Origin resolution keys on
    # the declared name `sentry_err`, bare or attribute.
    _write(tmp_path, "report.py", "def sentry_err(*a):\n    print(a)\n")
    caller = "import report\n\n\ndef h(session_token):\n    report.sentry_err(session_token)\n"
    _write(tmp_path, "caller.py", caller)
    r = _flake8(tmp_path, "caller.py", reporters="report.py#sentry_err")
    assert r.returncode == 1 and _TBX009 in r.stdout, f"{r.stdout}\n{r.stderr}"


def test_clean_args_with_dedupkey_literal_clean(tmp_path):
    # `dedup-key-abc123` contains `key` but is a string literal, not identified.
    r = _lint_reporter(
        tmp_path, 'def h(err):\n    report_it("revoke failed", err, "dedup-key-abc123")\n'
    )
    assert r.returncode == 0, f"{r.stdout}\n{r.stderr}"


def test_stopword_in_string_literal_message_clean(tmp_path):
    r = _lint_reporter(tmp_path, 'def h():\n    report_it("revoke failed -- live token leak")\n')
    assert r.returncode == 0, f"{r.stdout}\n{r.stderr}"


def test_non_reporter_with_secret_arg_clean(tmp_path):
    # Origin gate: only a declared reporter is scanned; log_error is not one.
    src = (
        "def report_it(*a):\n    print(a)\n\n\n"
        "def log_error(*a):\n    print(a)\n\n\n"
        "def h(auth_token):\n    log_error(auth_token)\n"
    )
    _write(tmp_path, "v.py", src)
    r = _flake8(tmp_path, "v.py", reporters="v.py#report_it")
    assert r.returncode == 0, f"{r.stdout}\n{r.stderr}"


def test_secret_named_dict_string_key_clean(tmp_path):
    # A string-literal key named like a secret does not count; the value is clean.
    r = _lint_reporter(tmp_path, 'def h(ok_value):\n    report_it("msg", {"token": ok_value})\n')
    assert r.returncode == 0, f"{r.stdout}\n{r.stderr}"
