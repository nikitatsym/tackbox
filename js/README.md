...
| Rule                                  | Summary                                                                          |
|---------------------------------------|----------------------------------------------------------------------------------|
| `tackbox/no-swallow-catch`            | `catch` must throw, call a reporter, or carry `// no-sentry: <reason>` above try |
| `tackbox/no-swallow-promise-catch`    | `.catch(handler)` must throw, call a reporter, or carry the marker               |
| `tackbox/no-console-error`            | `console.error` is banned; use `reportError`                                     |
| `tackbox/valid-error-report`          | static 15-200 char msg, cause non-null, tags non-empty, dedupKey required        |
| `tackbox/valid-dedup-key`             | dedupKey must be a static literal in `area.suffix[:identifier]` form             |
| `tackbox/no-secret-in-report`         | reporter args must not reference `token`/`password`/`key`/`secret`/`cookie`      |
| `tackbox/valid-throw-error`           | `throw new Error(msg)` msg must be static 15-200 chars                           |
